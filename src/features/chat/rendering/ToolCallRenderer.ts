...
function appendToolLink(parent: HTMLElement, title: string, url: string): void {
  const linkEl = parent.createEl('a', { cls: 'vauex-tool-link' });
  linkEl.setAttribute('href', url);
  linkEl.setAttribute('target', '_blank');
  linkEl.setAttribute('rel', 'noopener noreferrer');

  const iconEl = linkEl.createSpan({ cls: 'vauex-tool-link-icon' });
  setIcon(iconEl, 'external-link');

  linkEl.createSpan({ cls: 'vauex-tool-link-title', text: title });
}
...
function renderWebSearchActionExpanded(container: HTMLElement, input: Record<string, unknown>): boolean {
  ...
  const linesEl = container.createDiv({ cls: 'vauex-tool-lines' });

  switch (data.actionType) {
    case 'open_page':
      linesEl.createDiv({ cls: 'vauex-tool-line', text: 'Open page' });
      if (data.url) {
        appendToolLink(linesEl, data.url, data.url);
      } else {
        linesEl.createDiv({ cls: 'vauex-tool-line', text: 'URL unavailable' });
      }
      return true;

    case 'find_in_page':
      linesEl.createDiv({ cls: 'vauex-tool-line', text: 'Find in page' });
      if (data.url) {
        appendToolLink(linesEl, data.url, data.url);
      } else {
        linesEl.createDiv({ cls: 'vauex-tool-line', text: 'URL unavailable' });
      ...
      if (data.pattern) {
        linesEl.createDiv({ cls: 'vauex-tool-line', text: `Pattern: ${data.pattern}` });
      }
      return true;
    ...
    default: {
      ...
      for (const query of alternateQueries.slice(0, 4)) {
        linesEl.createDiv({ cls: 'vauex-tool-line', text: `Alt query: ${query}` });
      }
      if (alternateQueries.length > 4) {
        linesEl.createDiv({
          cls: 'vauex-tool-truncated',
...
function renderWebSearchExpanded(
  ...
  if (parsed && parsed.links.length > 0) {
    const linksEl = container.createDiv({ cls: 'vauex-tool-lines' });
    for (const link of parsed.links) {
      appendToolLink(linksEl, link.title, link.url);
    }

    if (parsed.summary) {
      const summaryEl = container.createDiv({ cls: 'vauex-tool-web-summary' });
  ...
  container.createDiv({ cls: 'vauex-tool-empty', text: 'No result' });
}

function renderFileSearchExpanded(container: HTMLElement, result: string): void {
  const lines = result.split(/\r?\n/).filter(line => line.trim());
  if (lines.length === 0) {
    container.createDiv({ cls: 'vauex-tool-empty', text: 'No matches found' });
    return;
  }
...
function renderLinesExpanded(
  ...
  for (const line of displayLines) {
    ...
    const lineEl = linesEl.createDiv({ cls: 'vauex-tool-line' });
...
function renderToolSearchExpanded(container: HTMLElement, result: string): void {
  ...
  for (const name of toolNames) {
    const lineEl = container.createDiv({ cls: 'vauex-tool-search-item' });
    const iconEl = lineEl.createSpan({ cls: 'vauex-tool-search-icon' });
...
function renderApplyPatchExpanded(
  ...
  if (parsedDiffs.length > 0) {
    for (const fileDiff of parsedDiffs) {
      const sectionEl = container.createDiv({ cls: 'vauex-tool-patch-section' });
      ...
      const pathText = fileDiff.movedTo
        ...
        : fileDiff.filePath;
      sectionEl.createDiv({
        cls: 'vauex-tool-patch-header',
        text: `${fileDiff.operation}: ${pathText}${statsSuffix}`,
      });

      if (fileDiff.operation === 'delete' && fileDiff.diffLines.length === 0) {
        sectionEl.createDiv({ cls: 'vauex-tool-empty', text: 'File deleted' });
        continue;
      }

      if (fileDiff.diffLines.length === 0) {
        sectionEl.createDiv({ cls: 'vauex-tool-empty', text: 'No textual diff available' });
        continue;
      }

      const diffRow = sectionEl.createDiv({ cls: 'vauex-write-edit-diff-row' });
      const diffEl = diffRow.createDiv({ cls: 'vauex-write-edit-diff' });
  ...
  if (changes.length > 0) {
    ...
    for (const change of changes) {
      ...
      if (!path) continue;
      linesEl.createDiv({ cls: 'vauex-tool-line', text: `${kind}: ${path}` });
    }
    return;
  }
...
function resetStatusElement(statusEl: HTMLElement, statusClass: string, ariaLabel: string): void {
  statusEl.className = 'vauex-tool-status';
  statusEl.empty();
...
export function renderTodoWriteResult(
  ...
  container.empty();
  container.addClass('vauex-todo-panel-content');
  container.addClass('vauex-todo-list-container');

  const todos = input.todos as TodoItem[] | undefined;
  if (!todos || !Array.isArray(todos)) {
    const item = container.createSpan({ cls: 'vauex-tool-result-item' });
...
function createToolElementStructure(
  ...
  const toolEl = parentEl.createDiv({ cls: 'vauex-tool-call' });
  if (toolCall.name === TOOL_BASH) {
    toolEl.addClass('vauex-tool-call-bash');
  }

  const header = toolEl.createDiv({ cls: 'vauex-tool-header' });
  header.setAttribute('tabindex', '0');
  header.setAttribute('role', 'button');

  const iconEl = header.createSpan({ cls: 'vauex-tool-icon' });
  iconEl.setAttribute('aria-hidden', 'true');
  setToolIcon(iconEl, toolCall.name);

  const nameEl = header.createSpan({ cls: 'vauex-tool-name' });
  nameEl.setText(getToolName(toolCall.name, toolCall.input));

  const summaryEl = header.createSpan({ cls: 'vauex-tool-summary' });
  ...
  const statusEl = header.createSpan({ cls: 'vauex-tool-status' });

  const content = toolEl.createDiv({ cls: 'vauex-tool-content' });
...
function renderAskUserQuestionResult(container: HTMLElement, toolCall: ToolCallInfo): boolean {
  ...
  const reviewEl = container.createDiv({ cls: 'vauex-ask-review' });
  for (let i = 0; i < questions.length; i++) {
    ...
    pairEl.createDiv({ text: `${i + 1}.`, cls: 'vauex-ask-review-num' });
    ...
    bodyEl.createDiv({
      ...
      cls: answer ? 'vauex-ask-review-a-text' : 'vauex-ask-review-empty',
    });
  }
...
function renderAskUserQuestionFallback(container: HTMLElement, toolCall: ToolCallInfo, initialText?: string): void {
  ...
  for (let questionIndex = 0; questionIndex < questions.length; questionIndex++) {
    ...
    const reviewEl = container.createDiv({ cls: 'vauex-ask-review' });
    const pairEl = reviewEl.createDiv({ cls: 'vauex-ask-review-pair' });
    pairEl.createDiv({ text: `${questionIndex + 1}.`, cls: 'vauex-ask-review-num' });
    const bodyEl = pairEl.createDiv({ cls: 'vauex-ask-review-body' });
    bodyEl.createDiv({ text: question.question, cls: 'vauex-ask-review-q-text' });

    if (!Array.isArray(question.options) || question.options.length === 0) {
      bodyEl.createDiv({ cls: 'vauex-ask-review-empty', text: 'No options recorded' });
      continue;
    }

    const listEl = bodyEl.createDiv({ cls: 'vauex-ask-list' });
...
function renderAskUserQuestionOption(
  ...
  const itemEl = parentEl.createDiv({ cls: 'vauex-ask-item is-disabled' });

  if (isMultiSelect) {
    itemEl.createDiv({ cls: 'vauex-ask-check', text: '[ ] ' });
  } else {
    itemEl.createDiv({ cls: 'vauex-ask-item-num', text: `${optionIndex + 1}. ` });
  }

  const contentEl = itemEl.createDiv({ cls: 'vauex-ask-item-content' });
  const labelRowEl = contentEl.createDiv({ cls: 'vauex-ask-label-row' });
  labelRowEl.createDiv({ cls: 'vauex-ask-item-label', text: option.label });

  if (option.description) {
    contentEl.createDiv({ cls: 'vauex-ask-item-desc', text: option.description });
  }
}

function contentFallback(container: HTMLElement, text: string): void {
  const resultRow = container.createDiv({ cls: 'vauex-tool-result-row' });
  const resultText = resultRow.createSpan({ cls: 'vauex-tool-result-text' });
...
function renderBashContent(
  ...
  if (command) {
    const cmdEl = container.createDiv({ cls: 'vauex-tool-bash-command' });
...
function createCurrentTaskPreview(
  ...
  const currentTaskEl = header.createSpan({ cls: 'vauex-tool-current' });
...
export function updateToolCallResult(
  ...
  if (toolCall.name === TOOL_TODO_WRITE) {
    const statusEl = toolEl.querySelector('.vauex-tool-status') as HTMLElement;
    if (statusEl) {
      setTodoWriteStatus(statusEl, toolCall.input);
    }
    const content = toolEl.querySelector('.vauex-tool-content') as HTMLElement;
    if (content) {
      renderTodoWriteResult(content, toolCall.input);
    }
    const nameEl = toolEl.querySelector('.vauex-tool-name') as HTMLElement;
    if (nameEl) {
      nameEl.setText(getToolName(toolCall.name, toolCall.input));
    }
    const currentTaskEl = toolEl.querySelector('.vauex-tool-current') as HTMLElement;
    if (currentTaskEl) {
