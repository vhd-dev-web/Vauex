...
function diffToHtml(ops: DiffOp[]): string {
  return ops
    .map((op) => {
      ...
      switch (op.type) {
        case 'delete':
          return `<span class="vauex-diff-del">${escaped}</span>`;
        case 'insert':
          return `<span class="vauex-diff-ins">${escaped}</span>`;
        default:
          return escaped;
      }
    })
...
describe('InlineEditModal - Word-level Diff', () => {
  ...
  describe('diffToHtml', () => {
    ...
    it('should wrap deleted text with del class', () => {
      ...
      expect(html).toContain('vauex-diff-del');
    ...
    it('should wrap inserted text with ins class', () => {
      ...
      expect(html).toContain('vauex-diff-ins');
