...
export function createThinkingBlock(
  ...
  const wrapperEl = parentEl.createDiv({ cls: 'vauex-thinking-block' });

  // Header (clickable to expand/collapse)
  const header = wrapperEl.createDiv({ cls: 'vauex-thinking-header' });
  ...
  // Label with timer
  const labelEl = header.createSpan({ cls: 'vauex-thinking-label' });
  ...
  const contentEl = wrapperEl.createDiv({ cls: 'vauex-thinking-content' });
...
export function finalizeThinkingBlock(state: ThinkingBlockState): number {
  ...
  const header = state.wrapperEl.querySelector('.vauex-thinking-header');
  if (header) {
