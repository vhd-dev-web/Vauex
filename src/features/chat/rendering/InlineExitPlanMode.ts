...
export class InlineExitPlanMode {
  ...
  render(): void {
    ...
    const titleEl = this.rootEl.createDiv({ cls: 'vauex-plan-inline-title' });
    titleEl.setText('Plan complete');

    this.planContent = this.readPlanContent();
    if (this.planContent) {
      const contentEl = this.rootEl.createDiv({ cls: 'vauex-plan-content-preview' });
      if (this.renderContent) {
        void this.renderContent(contentEl, this.planContent);
      } else {
        contentEl.createDiv({ cls: 'vauex-plan-content-text', text: this.planContent });
      }
    } else if (this.planReadError) {
      this.rootEl.createDiv({
        cls: 'vauex-plan-content-preview vauex-plan-read-error',
    ...
    if (allowedPrompts && Array.isArray(allowedPrompts) && allowedPrompts.length > 0) {
      const permEl = this.rootEl.createDiv({ cls: 'vauex-plan-permissions' });
      permEl.createDiv({ text: 'Requested permissions:', cls: 'vauex-plan-permissions-label' });
      const listEl = permEl.createEl('ul', { cls: 'vauex-plan-permissions-list' });
    ...
    const newSessionRow = actionsEl.createDiv({ cls: 'vauex-ask-item' });
    newSessionRow.addClass('is-focused');
    newSessionRow.createSpan({ text: '\u203A', cls: 'vauex-ask-cursor' });
    newSessionRow.createSpan({ text: '1. ', cls: 'vauex-ask-item-num' });
    newSessionRow.createSpan({ text: 'Approve (new session)', cls: 'vauex-ask-item-label' });
    ...
    const approveRow = actionsEl.createDiv({ cls: 'vauex-ask-item' });
    approveRow.createSpan({ text: '\u00A0', cls: 'vauex-ask-cursor' });
    approveRow.createSpan({ text: '2. ', cls: 'vauex-ask-item-num' });
    approveRow.createSpan({ text: 'Approve (current session)', cls: 'vauex-ask-item-label' });
    ...
    const feedbackRow = actionsEl.createDiv({ cls: 'vauex-ask-item vauex-ask-custom-item' });
    feedbackRow.createSpan({ text: '\u00A0', cls: 'vauex-ask-cursor' });
    feedbackRow.createSpan({ text: '3. ', cls: 'vauex-ask-item-num' });
