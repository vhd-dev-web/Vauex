...
import { type EnvironmentScope, type EnvSnippet, VIEW_TYPE_CLAUDIAN } from '../../../core/types';
...
export class EnvSnippetModal extends Modal {
  ...
  onOpen() {
    ...
    this.modalEl.addClass('vauex-env-snippet-modal');
    ...
    const renderContextLimitFields = () => {
      ...
      for (const modelId of uniqueModelIds) {
        const row = contextLimitsContainer.createDiv({ cls: 'vauex-snippet-limit-row' });
        row.createSpan({ text: modelId, cls: 'vauex-snippet-limit-model' });
        row.createSpan({ cls: 'vauex-snippet-limit-spacer' });

        const input = row.createEl('input', {
          type: 'text',
          placeholder: '200k',
          cls: 'vauex-snippet-limit-input',
        });
    ...
    envVarsSetting.settingEl.addClass('vauex-env-snippet-setting');
    envVarsSetting.controlEl.addClass('vauex-env-snippet-control');

    contextLimitsContainer = contentEl.createDiv({ cls: 'vauex-snippet-context-limits' });
    renderContextLimitFields();

    const buttonContainer = contentEl.createDiv({ cls: 'vauex-snippet-buttons' });
...
export class EnvSnippetManager {
  ...
  private render() {
    ...
    const headerEl = this.containerEl.createDiv({ cls: 'vauex-snippet-header' });
    headerEl.createSpan({ text: t('settings.envSnippets.name'), cls: 'vauex-snippet-label' });
    ...
    if (snippets.length === 0) {
      const emptyEl = this.containerEl.createDiv({ cls: 'vauex-snippet-empty' });
      emptyEl.setText(t('settings.envSnippets.noSnippets'));
      return;
    }

    const listEl = this.containerEl.createDiv({ cls: 'vauex-snippet-list' });

    for (const snippet of snippets) {
      const itemEl = listEl.createDiv({ cls: 'vauex-snippet-item' });

      const infoEl = itemEl.createDiv({ cls: 'vauex-snippet-info' });

      const nameEl = infoEl.createDiv({ cls: 'vauex-snippet-name' });
      nameEl.setText(snippet.name);

      if (snippet.description) {
        const descEl = infoEl.createDiv({ cls: 'vauex-snippet-description' });
        descEl.setText(snippet.description);
      }

      const actionsEl = itemEl.createDiv({ cls: 'vauex-snippet-actions' });
  ...
  private syncTextareaValue(scope: EnvironmentScope, value: string): void {
    const selector = `.vauex-settings-env-textarea[data-env-scope="${scope}"]`;
