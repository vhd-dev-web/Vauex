...
export class MessageRenderer {
  ...
  addMessage(msg: ChatMessage): HTMLElement {
    ...
    const msgEl = this.messagesEl.createDiv({
      cls: `vauex-message vauex-message-${msg.role}`,
      attr: {
    ...
    const contentEl = msgEl.createDiv({ cls: 'vauex-message-content', attr: { dir: 'auto' } });

    if (msg.role === 'user') {
      const textToShow = msg.displayContent ?? msg.content;
      if (textToShow) {
        const textEl = contentEl.createDiv({ cls: 'vauex-text-block' });
  ...
  updateLiveUserMessage(msg: ChatMessage): void {
    ...
    const contentEl = msgEl.querySelector('.vauex-message-content') as HTMLElement | null;
    if (!contentEl) {
      return;
    }
    ...
    const toolbar = msgEl.querySelector('.vauex-user-msg-actions') as HTMLElement | null;
    if (toolbar) {
      toolbar.querySelectorAll('.vauex-user-msg-copy-btn').forEach((el) => el.remove());
    }
  ...
  renderMessages(
    ...
    const newWelcomeEl = this.messagesEl.createDiv({ cls: 'vauex-welcome' });
    newWelcomeEl.createDiv({ cls: 'vauex-welcome-greeting', text: getGreeting() });
  ...
  private renderInterruptMessage(): void {
    const msgEl = this.messagesEl.createDiv({ cls: 'vauex-message vauex-message-assistant' });
    const contentEl = msgEl.createDiv({ cls: 'vauex-message-content', attr: { dir: 'auto' } });
    this.appendInterruptIndicator(contentEl);
  }
  ...
  private appendInterruptIndicator(contentEl: HTMLElement): void {
    const textEl = contentEl.createDiv({ cls: 'vauex-text-block' });
    textEl.innerHTML = '<span class="vauex-interrupted">Interrupted</span> <span class="vauex-interrupted-hint">· What should Vauex do instead?</span>';
  }
  ...
  private renderAssistantContent(msg: ChatMessage, contentEl: HTMLElement): void {
    if (msg.contentBlocks && msg.contentBlocks.length > 0) {
      ...
      for (const block of msg.contentBlocks) {
        if (block.type === 'thinking') {
          ...
          const boundaryEl = contentEl.createDiv({ cls: 'vauex-compact-boundary' });
          boundaryEl.createSpan({ cls: 'vauex-compact-boundary-label', text: 'Conversation compacted' });
    ...
    if (msg.durationSeconds && msg.durationSeconds > 0 && !hasCompactBoundary) {
      ...
      const footerEl = contentEl.createDiv({ cls: 'vauex-response-footer' });
      footerEl.createSpan({
        text: `* ${flavorWord} for ${formatDurationMmSs(msg.durationSeconds)}`,
        cls: 'vauex-baked-duration',
      });
    }
  }
  ...
  renderMessageImages(containerEl: HTMLElement, images: ImageAttachment[]): void {
    const imagesEl = containerEl.createDiv({ cls: 'vauex-message-images' });

    for (const image of images) {
      const imageWrapper = imagesEl.createDiv({ cls: 'vauex-message-image' });
  ...
  showFullImage(image: ImageAttachment): void {
    ...
    const overlay = document.body.createDiv({ cls: 'vauex-image-modal-overlay' });
    const modal = overlay.createDiv({ cls: 'vauex-image-modal' });
    ...
    const closeBtn = modal.createDiv({ cls: 'vauex-image-modal-close' });
    closeBtn.setText('\u00D7');
  ...
  async renderContent(el: HTMLElement, markdown: string): Promise<void> {
    ...
    try {
      ...
      el.querySelectorAll('pre').forEach((pre) => {
        ...
        if (pre.parentElement?.classList.contains('vauex-code-wrapper')) return;

        // Create wrapper
        const wrapper = createEl('div', { cls: 'vauex-code-wrapper' });
  ...
  addTextCopyButton(textEl: HTMLElement, markdown: string): void {
    const copyBtn = textEl.createSpan({ cls: 'vauex-text-copy-btn' });
  ...
  refreshActionButtons(msg: ChatMessage, allMessages?: ChatMessage[], index?: number): void {
    ...
    if (this.rewindCallback && !msgEl.querySelector('.vauex-message-rewind-btn')) {
      this.addRewindButton(msgEl, msg.id);
    }
    if (this.forkCallback && !msgEl.querySelector('.vauex-message-fork-btn')) {
      this.addForkButton(msgEl, msg.id);
    }
  ...
  private cleanupLiveMessageEl(msgId: string, msgEl: HTMLElement): void {
    const needsRewind = this.rewindCallback && !msgEl.querySelector('.vauex-message-rewind-btn');
    const needsFork = this.forkCallback && !msgEl.querySelector('.vauex-message-fork-btn');
  ...
  private getOrCreateActionsToolbar(msgEl: HTMLElement): HTMLElement {
    const existing = msgEl.querySelector('.vauex-user-msg-actions') as HTMLElement | null;
    if (existing) return existing;
    return msgEl.createDiv({ cls: 'vauex-user-msg-actions' });
  }

  private addUserCopyButton(msgEl: HTMLElement, content: string): void {
    const toolbar = this.getOrCreateActionsToolbar(msgEl);
    const copyBtn = toolbar.createSpan({ cls: 'vauex-user-msg-copy-btn' });
    copyBtn.innerHTML = MessageRenderer.COPY_ICON;
  ...
  private addRewindButton(msgEl: HTMLElement, messageId: string): void {
    ...
    const toolbar = this.getOrCreateActionsToolbar(msgEl);
    const btn = toolbar.createSpan({ cls: 'vauex-message-rewind-btn' });
  ...
  private addForkButton(msgEl: HTMLElement, messageId: string): void {
    ...
    const toolbar = this.getOrCreateActionsToolbar(msgEl);
    const btn = toolbar.createSpan({ cls: 'vauex-message-fork-btn' });
    if (toolbar.firstChild !== btn) toolbar.insertBefore(btn, toolbar.firstChild);
