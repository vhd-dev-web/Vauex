...
export class ClaudianView extends ItemView {
  ...
  async onOpen() {
    ...
    this.viewContainerEl.addClass('vauex-container');

    const header = this.viewContainerEl.createDiv({ cls: 'vauex-header' });
    this.buildHeader(header);

    this.navRowContent = this.buildNavRowContent();
    this.tabContentEl = this.viewContainerEl.createDiv({ cls: 'vauex-tab-content-container' });
  ...
  private buildHeader(header: HTMLElement) {
    ...
    this.titleSlotEl = header.createDiv({ cls: 'vauex-title-slot' });

    // Logo (hidden when 2+ tabs) — populated by syncHeaderLogo()
    this.logoEl = this.titleSlotEl.createSpan({ cls: 'vauex-logo' });
    this.syncHeaderLogo(DEFAULT_CHAT_PROVIDER_ID);

    // Title text (hidden in header mode when 2+ tabs)
    this.titleTextEl = this.titleSlotEl.createEl('h4', { text: 'Vauex', cls: 'vauex-title-text' });

    // Header actions container (for header mode - initially hidden)
    this.headerActionsEl = header.createDiv({ cls: 'vauex-header-actions vauex-header-actions-slot' });
  ...
  private buildNavRowContent(): HTMLElement {
    ...
    this.tabBarContainerEl.className = 'vauex-tab-bar-container';
    ...
    this.headerActionsContent.className = 'vauex-header-actions';

    // New tab button (plus icon)
    const newTabBtn = this.headerActionsContent.createDiv({ cls: 'vauex-header-btn vauex-new-tab-btn' });
    ...
    const newBtn = this.headerActionsContent.createDiv({ cls: 'vauex-header-btn' });
    ...
    // History dropdown
    const historyContainer = this.headerActionsContent.createDiv({ cls: 'vauex-history-container' });
    const historyBtn = historyContainer.createDiv({ cls: 'vauex-header-btn' });
    setIcon(historyBtn, 'history');
    historyBtn.setAttribute('aria-label', 'Chat history');

    this.historyDropdown = historyContainer.createDiv({ cls: 'vauex-history-menu' });
  ...
  updateLayoutForPosition(): void {
    ...
    this.viewContainerEl.toggleClass('vauex-container--header-mode', isHeaderMode);
