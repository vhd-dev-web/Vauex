...
describe('SessionStorage', () => {
  ...
  describe('SESSIONS_PATH', () => {
    it('should be .vauex/sessions', () => {
      expect(SESSIONS_PATH).toBe('.vauex/sessions');
    });
  });
  ...
  describe('getMetadataPath', () => {
    it('returns correct file path for session id', () => {
      ...
      expect(path).toBe('.vauex/sessions/session-abc.meta.json');
    });
  });
  ...
  describe('saveMetadata', () => {
    it('serializes metadata to JSON and writes to file', async () => {
      ...
      expect(mockAdapter.write).toHaveBeenCalledWith(
        '.vauex/sessions/session-456.meta.json',
        expect.any(String)
      );
  ...
  describe('loadMetadata', () => {
    ...
    it('loads legacy metadata and copies it to .vauex without deleting the legacy file', async () => {
      const metadata = {
        id: 'session-legacy',
      ...
      expect(mockAdapter.write).toHaveBeenCalledWith(
        '.vauex/sessions/session-legacy.meta.json',
        expect.any(String),
      );
  ...
  describe('listAllConversations - provider routing', () => {
    it('preserves providerId from metadata', async () => {
      mockAdapter.listFiles.mockResolvedValue([
        '.vauex/sessions/claude-session.meta.json',
        '.vauex/sessions/codex-session.meta.json',
      ]);
  ...
  describe('deleteMetadata', () => {
    it('deletes the meta.json file', async () => {
      ...
      expect(mockAdapter.delete).toHaveBeenCalledWith('.vauex/sessions/session-del.meta.json');
    });
  });
  ...
  describe('listMetadata', () => {
    it('returns metadata for .meta.json files', async () => {
      mockAdapter.listFiles.mockResolvedValue([
        '.vauex/sessions/native-1.meta.json',
        '.vauex/sessions/native-2.meta.json',
      ]);
    ...
    it('skips files that fail to load', async () => {
      mockAdapter.listFiles.mockResolvedValue([
        '.vauex/sessions/good.meta.json',
        '.vauex/sessions/bad.meta.json',
      ]);
  ...
  describe('listAllConversations', () => {
    it('returns metadata from listMetadata as ConversationMeta[]', async () => {
      mockAdapter.listFiles.mockResolvedValue([
        '.vauex/sessions/session-1.meta.json',
        '.vauex/sessions/session-2.meta.json',
      ]);
    ...
    it('preserves titleGenerationStatus', async () => {
      mockAdapter.listFiles.mockResolvedValue([
        '.vauex/sessions/session-status.meta.json',
      ]);
