...
export class MentionDropdownController {
  ...
  constructor(
    ...
    this.dropdown = new SelectableDropdown<MentionItem>(this.containerEl, {
      listClassName: 'vauex-mention-dropdown',
      itemClassName: 'vauex-mention-item',
      emptyClassName: 'vauex-mention-empty',
      fixed: this.fixed,
      fixedClassName: 'vauex-mention-dropdown-fixed',
    });
  }
  ...
  private renderMentionDropdown(): void {
    this.dropdown.render({
      ...
      renderItem: (item, itemEl) => {
        const iconEl = itemEl.createSpan({ cls: 'vauex-mention-icon' });
        switch (item.type) {
          case 'mcp-server':
        ...
        const textEl = itemEl.createSpan({ cls: 'vauex-mention-text' });

        switch (item.type) {
          case 'mcp-server':
            textEl.createSpan({ cls: 'vauex-mention-name' }).setText(`@${item.name}`);
            break;
          case 'agent-folder':
            textEl.createSpan({
              cls: 'vauex-mention-name vauex-mention-name-agent-folder',
            }).setText(`@${item.name}/`);
            break;
          case 'agent': {
            // Show ID (which is namespaced for plugin agents) for consistency with inserted text
            textEl.createSpan({
              cls: 'vauex-mention-name vauex-mention-name-agent',
            }).setText(`@${item.id}`);
            if (item.description) {
              textEl.createSpan({ cls: 'vauex-mention-agent-desc' }).setText(item.description);
            }
            break;
          }
          case 'context-folder':
            textEl.createSpan({
              cls: 'vauex-mention-name vauex-mention-name-folder',
            }).setText(`@${item.name}/`);
            break;
          case 'context-file':
            textEl.createSpan({
              cls: 'vauex-mention-name vauex-mention-name-context',
            }).setText(item.name);
            break;
          case 'folder':
            textEl.createSpan({
            ...
            break;
          default:
            textEl.createSpan({ cls: 'vauex-mention-path' }).setText(item.path || item.name);
        }
      },
