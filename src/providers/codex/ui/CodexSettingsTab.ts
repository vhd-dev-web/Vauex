...
export const codexSettingsTabRenderer: ProviderSettingsTabRenderer = {
  render(container, context) {
    ...
    const resolvedCliPath = codexWorkspace.cliResolver.resolveFromSettings(settingsBag);
    ...
    setupStatus.createEl('p', {
      ...
      text: resolvedCliPath
        ? `Codex CLI detected: ${resolvedCliPath}`
        : 'Codex CLI was not detected. Vauex needs the Codex CLI before chat, inline edit, skills, and subagents can run.',
    });
    if (!resolvedCliPath) {
      const setupSteps = setupStatus.createEl('ol', { cls: 'setting-item-description' });
      setupSteps.createEl('li', { text: 'Install and authenticate the Codex CLI.' });
      const restartStep = setupSteps.createEl('li');
      restartStep.appendText('Restart Obsidian so Vauex can see your updated ');
      restartStep.createEl('code', { text: 'PATH' });
      restartStep.appendText('.');
      setupSteps.createEl('li', { text: 'If auto-detection still fails, set the CLI path below.' });
    }
    updateCodexProviderSettings(settingsBag, { enabled: true });

    if (isWindowsHost) {
      new Setting(container)
        .setName('Installation method')
        .setDesc('How Vauex should launch Codex on Windows. Native Windows uses a Windows executable path. WSL launches the Linux CLI inside a selected distro.')
    ...
    mcpDesc.appendText(' and they will be available in Vauex. ');
    mcpDesc.createEl('a', {
      text: 'Learn more',
