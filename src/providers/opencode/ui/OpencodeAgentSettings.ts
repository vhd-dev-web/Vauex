...
export class OpencodeAgentSettings {
  ...
  async render(): Promise<void> {
    ...
    headerEl.createSpan({ text: 'OpenCode Subagents', cls: 'vauex-sp-label' });
  ...
  private renderItem(listEl: HTMLElement, agent: OpencodeAgentDefinition): void {
    ...
    headerRow.createSpan({
      text: 'subagent',
      cls: 'vauex-slash-item-badge',
    });

    if (agent.model) {
      headerRow.createSpan({ text: agent.model, cls: 'vauex-slash-item-badge' });
    }
