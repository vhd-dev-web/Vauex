...
class ForkTargetModal extends Modal {
  ...
  onOpen() {
    ...
    this.modalEl.addClass('vauex-fork-target-modal');

    const list = this.contentEl.createDiv({ cls: 'vauex-fork-target-list' });
  ...
  private createOption(container: HTMLElement, target: ForkTarget, label: string): void {
    const item = container.createDiv({ cls: 'vauex-fork-target-option', text: label });
